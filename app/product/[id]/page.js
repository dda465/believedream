...
export default function ProductDetail({ params }) {
  ...
  if (!p) return (
    <div className="text-center py-20 px-5">
      ...
      <Link href="/" className="text-[13px] text-[var(--accent)] font-bold mt-3 inline-block">← 홈으로</Link>
    </div>
  );
  ...
  return (
    <div className="w-full bg-[var(--bg-main)] pb-[90px]">
      ...
      {/* 이미지 영역 */}
      <div className="bg-[var(--bg-sub)] h-[240px] flex items-center justify-center relative">
        <span className="text-[80px]">{p.emoji}</span>
        {p.todayInstall && (
          <span className="absolute top-3 right-3 text-[10px] font-bold bg-[var(--success)] text-white px-3 py-1 rounded-full shadow-sm">⚡ 당일 설치 가능</span>
        )}
      </div>
      ...
      <div className="px-5 py-5 bg-white">
        <div className="flex items-center gap-1.5 mb-2">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${condStyle[p.condition]}`}>중고 {p.condition}</span>
          {p.buyout && <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-[var(--purple-soft)] text-[var(--purple)]">인수가능</span>}
        </div>
      ...
      <div className="px-5 py-5 bg-white">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <div key={opt.period} className={`relative text-center py-2.5 rounded-[var(--radius-md)] border transition-all ${
              opt.active ? 'border-[var(--accent)] bg-[var(--accent-soft)]' : 'border-[var(--border)] bg-white'
            }`}>
              {opt.active && <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] font-bold bg-[var(--accent)] text-white px-2 py-0.5 rounded-full">추천</span>}
              <p className="text-[11px] text-[var(--text-light)]">{opt.period}</p>
        ...
        {p.buyout && (
          <div className="mt-3 bg-[var(--bg-sub)] rounded-[var(--radius-md)] p-3.5 flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-[var(--text-light)]">약정 후 잔여가만 납부</p>
            </div>
      ...
      {/* 상태 */}
      <div className="px-5 py-5 bg-white">
        <h3 className="text-[14px] font-black text-[var(--text-dark)] mb-2">📋 상품 상태</h3>
      ...
      <div className="px-5 py-5 bg-white">
        ...
        <div className="bg-[var(--bg-main)] border border-[var(--border-light)] rounded-[var(--radius-md)] p-4">
          <p className="text-[14px] font-bold text-[var(--text-dark)]">{p.store}</p>
          <p className="text-[11px] text-[var(--text-light)] mt-0.5">📍 {p.district} {p.area}</p>
          <div className="flex items-center gap-3 mt-2.5 pt-2.5 border-t border-[var(--border-light)]">
            <span className="text-[11px] text-[var(--text-light)]">{p.todayInstall ? '⚡ 당일 설치' : '📅 예약 설치'}</span>
            <span className="text-[11px] text-[var(--text-light)]">{p.onlinePay ? '💳 온라인 결제' : '🏪 현장 결제'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 max-w-[480px] w-full z-50 bg-white border-t border-[var(--border-light)] px-5 py-3 shadow-nav">
        <div className="flex gap-2">
          <button className="w-[48px] h-[48px] border border-[var(--border)] rounded-[var(--radius-md)] flex items-center justify-center hover:bg-[var(--bg-sub)] transition-colors text-[18px] flex-shrink-0">💬</button>
          <Link href={`/apply?product=${p.id}`}
            className="flex-1 h-[48px] bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white font-bold rounded-[var(--radius-md)] text-[14px] flex items-center justify-center transition-all active:scale-[0.98] shadow-lg">
            렌탈 신청하기
          </Link>
        </div>
      </div>
